...
export type EventContainer = Window | HTMLElement | XMLDocument

export type EventDriverContainer = HTMLElement | XMLDocument
...
export class Event extends Subscribable<ICustomEvent<any>> {
    ...
    constructor(props?: IEventProps) {
        super()
        if (_.isArray(props?.effects)) {
            props?.effects.forEach((plugin) => {
                plugin(this)
            })
        }
        if (_.isArray(props?.drivers)) {
    ...
    attachEvents(
        ...
        if (_.get(container,ATTACHED_SYMBOL)) return
