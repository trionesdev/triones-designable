...
import { CursorType, DEFAULT_SCALE_SIZE, ScreenType } from "@alkaid/core";
import {
  useCursor,
  useHistory,
  useScreen,
...
export const DesignerToolsWidget: React.FC<IDesignerToolsWidgetProps> =
  observer((props) => {
    ...
    const renderScaleController = () => {
      ...
      if (screen.type !== ScreenType.Scale) return null;
      if (screen.width === "100%" || screen.height === "100%") {
        screen.setSize(DEFAULT_SCALE_SIZE.width, DEFAULT_SCALE_SIZE.height);
      }
      return (
        <Fragment>
          <InputNumber
    ...
    const renderScreenTypeController = () => {
      ...
      return (
        <Button.Group size="small" style={{ marginRight: 20 }}>
          ...
          <Button
            size="small"
            disabled={screen.type === ScreenType.Scale}
            onClick={() => {
              screen.setType(ScreenType.Scale);
            }}
          >
            缩放画布
            {/* <IconWidget infer="Down" /> */}
          </Button>
        </Button.Group>
      );
    };
