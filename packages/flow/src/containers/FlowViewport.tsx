...
export const FlowViewport = observer(() => {
  ...
  const [{}, drop] = useDrop(
    () => ({
      ...
      drop: (node: TreeNode, monitor) => {
        ...
        const localPoint = viewport.graph.pageToLocal(
          pagePoint?.x!,
          pagePoint?.y!,
        );
  ...
  useEffect(() => {
    ...
    graphInstance.on('cell:contextmenu', ({ e, x, y, cell, view }) => {
      ...
      const items = viewport.engine.contextMenuService(
        type,
        cell,
        graphInstance,
      );
  ...
  return wrapSSR(
    <div className={cls(prefix, hashId)} ref={drop}>
